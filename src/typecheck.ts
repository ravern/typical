...
export function substitute(ctx: Context, monotype: Monotype): Monotype {
...
// the given context.
export function unify(ctx: Context, leftRawMonotype: Monotype, rightRawMonotype: Monotype): Context {
...
export function infer(ctx: Context, expr: Expr): [Context, Monotype] {
  switch (expr.type) {
...
export function typecheck(expr: Expr): string {
  const [ctx, type] = infer(empty(), expr);
  return show(substitute(ctx, type));
}
